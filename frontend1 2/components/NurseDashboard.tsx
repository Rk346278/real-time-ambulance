...
export const NurseDashboard: React.FC<NurseDashboardProps> = ({ onBack }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!patientName || !age || !notes) {
            setMessage({ text: 'Please fill patient name, age, and notes.', type: 'error' });
            return;
        }
    ...
    return (
        <div className="max-w-2xl mx-auto bg-gray-800 p-8 rounded-xl shadow-2xl shadow-indigo-500/10 border border-gray-700">
            <div className="relative mb-6">
                <button
                    onClick={onBack}
                    className="absolute left-0 top-1/2 -translate-y-1/2 text-indigo-400 hover:text-indigo-300 transition-colors"
                >
                    &larr; Home
                </button>
